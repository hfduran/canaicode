...
class MetricsCalculator:
    ...
    def calculate_gross_productivity_grouped_by(
        ...
        if not set(group_by).issubset(allowed):
            raise ValueError(
                f"Invalid group_by columns: {group_by}. Allowed: {allowed}"
            )

        df_commits = pd.DataFrame([c.model_dump() for c in commits])
        df_repos = pd.DataFrame([r.model_dump() for r in repositories]).rename(
        ...
        result = (
            df_commits.groupby(group_by)["total"]  # type: ignore
            .sum()
            .reset_index(name="gross_productivity")
        )

        return result

    @staticmethod
    def calculate_gross_use_of_AI_grouped_by(
        metrics: List[CopilotCodeMetrics], group_by: List[str]
    ) -> pd.DataFrame:
        ...
        result: pd.DataFrame = (
            ...
            .sum()
            .reset_index(name="gross_use_of_AI")
        )
        return result

    @staticmethod
    def calculate_relative_use_of_AI_grouped_by(
        metrics: List[CopilotCodeMetrics], group_by: List[str]
    ) -> pd.DataFrame:
        allowed = {"language", "team_id", "IDE", "copilot_model"}
        if not set(group_by).issubset(allowed):
        ...
        grouped["relative_use_of_AI"] = grouped.apply(  # type: ignore
            lambda row: row["lines_accepted"] / row["lines_suggested"]  # type: ignore
            if row["lines_suggested"] > 0
            else 0,
            axis=1,
        )
