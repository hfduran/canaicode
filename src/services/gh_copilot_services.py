...
from consumers.gh_copilot.gh_copilot_consumer import CopilotMetricsEntry, GhCopilotConsumer
...
class GhCopilotServices:
    ...
    def summarize_metrics(start_date: date, end_date: date) -> None:
                ...
                for model_data in models_data:
                    ...
                    for language_data in languages_data:
                        ...
                        if lang_name is None:
                            continue
                        language_loc_metrics.setdefault(
                            lang_name, 
                            LocMetrics()
                        )
        ...
        for lang, loc_metrics in language_loc_metrics.items():
            ...
            echo(f"  Total Code Lines Accepted: {loc_metrics.total_code_lines_accepted}")
            echo(f"  Total Code Lines Suggested: {loc_metrics.total_code_lines_suggested}")
            echo("---")
        echo()
